...
test('水晶招喚以逐張立繪與台詞揭示，並可跳過演出', () => {
  ...
  assert.match(html, /id="gacha-reveal"/); assert.match(html, /id="gacha-skip"[^>]*>SKIP/); assert.match(html, /tactical-pets\.js\?v=8/); assert.match(html, /js\/tactics\.js\?v=50/);
...
test('遠距攻擊從施術者本體出發，速度差可觸發閃避與未命中演出', () => {
  ...
  const source = readFileSync(join(root, 'js/tactics.js'), 'utf8');
  const battle = readFileSync(join(root, 'css/tactics-battle.css'), 'utf8');
  assert.match(html, /tactics-battle\.css\?v=13/); assert.match(html, /js\/tactics\.js\?v=50/);
