...
export function enrichPets(raw){return raw.map((p,i)=>{let role=ROLES[i%ROLES.length];let speed=80+(i*13)%55;return {...p,role,speed,model:`pets/pet-${String(i+1).padStart(2,'0')}`,modelIndex:i+1,skillEnergy:45+(i%3)*10,branchA:{name:p.name+'・烈',role:'強攻',bonus:'爆發傷害 +25%'},branchB:{name:p.name+'・靈',role:i%2?'支援':'守護',bonus:i%2?'能量恢復 +20%':'生命與防禦 +20%'}}})}
