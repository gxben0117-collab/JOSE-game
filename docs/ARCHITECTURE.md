...
3. 戰鬥控制器 `js/tactics.js`
   ...
   - 四方向動畫：全部 115 隻幻獸與 52 隻魔獸使用 `assets/animations/directional/` 的 6×12 動畫表；每個方向的待命、移動、攻擊各有 6 幀，路徑逐段切換 down／right／up／left，攻擊依目標主方位轉向。完整規則見 `docs/幻獸與魔獸四方向六幀動作圖規格.md`。
