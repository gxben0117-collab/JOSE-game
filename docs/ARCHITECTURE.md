...
更新日期：2026-07-18（V0.19／pkg 0.19.0）
...
3. 戰鬥控制器 `js/tactics.js`
   ...
   - 四方向動畫：全部 115 隻幻獸與 52 隻魔獸使用 `assets/animations/directional/` 的 6×12 動畫表；每個方向的待命、移動、攻擊各有 6 幀，路徑逐段切換 down／right／up／left，攻擊依目標主方位轉向。65 隻具有三階原畫的幻獸依 `unit.evolution` 改載 `-stage_2` 或 `-stage_3` 專屬圖集。完整規則見 `docs/幻獸與魔獸四方向六幀動作圖規格.md`。
