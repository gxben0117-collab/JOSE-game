...
3. 戰鬥控制器 `js/tactics.js`
   ...
   - 四方向動畫：初始四獸使用 `assets/animations/directional/` 的 6×12 動畫表；每個方向的待機、移動、攻擊各有 6 幀，路徑逐段切換 down／right／up／left，攻擊依目標主方位轉向。未完成四方向原畫的單位由 `FOUR_DIRECTION_UNITS` 控制回退舊 4×6 左右表。
