...
更新日期：2026-07-17（V0.10／pkg 0.9.0）
...
3. 戰鬥控制器 `js/tactics.js`
   ...
   - 空間系統：30×30 大地圖（`COLS/ROWS = 30`）、BFS 路徑（障礙物阻擋）、`reachableTiles()`、`lineClear()` 視線采樣——遠程／範圍技能需要通視。
   - 鏡頭系統：視窗取景約 12×10（`--cell` 固定尺寸＋`board-scroll` 視窗）；`focusCamera()` 平滑跟隨行動單位、`enableCameraDrag()` 拖曳平移（位移超過 8px 抑制點擊 180ms）、`renderMinimap()` 小地圖導航（敵我點位＋視角框，可點擊跳轉）。
   ...
   - 演出：爆擊（12%+速度優勢6%，1.5×）、狀態浮字、擊殺淡出、分級震動。
...
5. 驗證與輸出
   ...
   - `scripts/test.mjs`：39 項回歸（資料、敵人、關卡、障礙物、進度、三畫面、引擎特徵、10 場模擬）。
...
3. `reset()` 建立障礙物表與雙方單位，進入 `deploy` 階段；玩家可在左三欄自由擺位後開戰。
...
- 實測基準（headless 全速自動戰鬥、6 人編隊）：初始隊過 ember-1~3；ember-4 首領需養成；同層級隊可過 verdant-4；tide-4 需同層滿養成剋制隊；rift-4 需滿養成神獸隊。
