...
更新日期：2026-07-18（V0.17／pkg 0.17.0）
...
3. 戰鬥控制器 `js/tactics.js`
   ...
   - 編隊：`DEPLOY_CAPACITY = 25`；1×1 成本 1、2×2 成本 4、3×3 成本 3。存檔、編隊 UI 與戰場配置共用同一成本規則；敵方 `rosterFor()` 名冊展開 10〜30 隻，`enemyFormation()` 依 seed 分小隊散布，`squadActive()` 警戒圈（7 格）控制出擊；首領關斬首即勝。
